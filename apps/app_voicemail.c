...
static int leave_voicemail(struct ast_channel *chan, char *ext, struct leave_vm_options *options)
{
	...
	/* granig: */
	ast_log(LOG_WARNING, "granig: '%s' has uuid '%s'\n", ext, vmu->mailbox);
	...
	if (!ast_strlen_zero(prefile)) {
		...
		if (ast_fileexists(prefile, NULL, NULL) > 0) {
			...
			res = invent_message(chan, vmu->context, ext, vmu->dialed_num, ast_test_flag(options, OPT_BUSY_GREETING), ecodes);
		}
		DISPOSE(prefile, -1);
		if (res < 0) {
			if (option_debug)
			...
			while ((cmd > -1) && (cmd != 't') && (cmd != '#')) {
				...
				*/
				...
				default:
					...
					*/
					/* agranig: instead of block above: */
					if (!vms.starting) {
						cmd = ast_play_and_wait(chan, "vm-tohearenv");
					}
